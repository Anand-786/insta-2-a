    ...
    <div className='bg-white my-7 border rounded-sm'>
        ...
        {session && (
          <form className='flex items-center p-4'>
            ...
            <input type='text' value={comment} onChange={e => setComment(e.target.value)} className='border-none flex-1 focus:ring-0 outline-none' placeholder='Write a comment...' />
            <button disabled={!comment.trim()} onClick={sendComment} type='submit' className='font-semibold text-blue-500'>Post</button>
          </form>
        )}
    </div>
  )
